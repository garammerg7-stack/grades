...
function debugAI() {
    ...
    console.log("API Key Exists:", !!ASSISTANT_CONFIG.apiKey);
    console.log("API Key Length:", ASSISTANT_CONFIG.apiKey ? ASSISTANT_CONFIG.apiKey.length : 0);
...
async function processAIQuery(query) {
    ...
    try {
        ...
        const studentData = Object.values(COURSE_DATA).map(course => {
            if (!course.students) return null;
            const student = course.students.find(s => s.name && s.name.trim() === currentStudentName);
            if (student) {
                return {
                    course: course.title || "بدون عنوان",
                    grades: {
                        classwork: student.classwork || 0,
                        final: student.final || 0,
                        total: student.total || 0
                    },
                    attendance: (course.attendance || []).find(a => a.name && a.name.trim() === currentStudentName)?.sessions || []
                };
            }
            return null;
        }).filter(item => item !== null);

        // Collect Announcements with defensive checks
        ...
        const context = `
            ...
            صندوق المعرفة للمدرس: ${ASSISTANT_CONFIG.knowledgeBase || "لا يوجد حالياً"}
        `;
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [{
                        ...
                        ملاحظة هامة: إذا سألك الطالب عن درجاته أو غيابه، ابحث عنها في بيانات الطالب المزودة أدناه.
        ...
        if (!response.ok) {
            ...
            console.error("Gemini API HTTP Error:", response.status, errorRaw);
            let technicalDetail = "";
            try {
                const errJson = JSON.parse(errorRaw);
                technicalDetail = errJson.error?.message || "";
            } catch (e) { technicalDetail = errorRaw; }

            throw new Error(`API Error (${response.status}): ${technicalDetail}`);
        }
        ...
        if (result.candidates && result.candidates[0].content && result.candidates[0].content.parts) {
            ...
            throw new Error("لم يتمكن المساعد من توليد إجابة مناسبة. راجع تفاصيل الـ Console.");
        }
    } catch (e) {
        console.error('Final AI Error Catch:', e);
...
console.log("Portal Script v1.1 Loaded.");
init();
